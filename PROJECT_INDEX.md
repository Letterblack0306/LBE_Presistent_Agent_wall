...
| `.ui-preview/` | **Existing HTML/CSS/JavaScript candidate surface; not canonical by current gate**: `agent_cockpit.html` is served by `server.py` (`GET /`,`/cockpit`,`/workspace/tree`,`/roots`,`/inspect`,`/search`). The recovered `lbe-authority` bridge remains a pending integration seam. Projection/control only; no installed/live acceptance is claimed | LBE product-surface projection owner | `docs/contracts/LBE_HOME_PROVIDER_SURFACE_CONTRACT.md` | Frontend selection, bounded bridge wiring, and installed acceptance require explicit user intent, the active machine gate, and owner/index reconciliation; no provider, session, authorization, execution, receipt, evidence, persistence, or completion authority |
...
| `lbe`, `C:\LBE-TUI-Lab\cline\apps\cli`, `C:\LBE-TUI-Lab\run-cline-lbe.ps1` | **LBE-branded coding IDE CLI/TUI product entrypoint with embedded Cline mechanics**; Cline source/launcher is an internal client/mechanics dependency, not an independent final-product authority | LBE product entry/runtime with embedded Cline reasoning/client mechanics | `docs/CURRENT_STATUS.md`, `docs/IMPLEMENTATION_PLAN.md`, and `docs/acceptance/CURRENT_IMPLEMENTATION_GATE.md` | LBE owns identity, policy, authorization, execution, receipts, evidence, persistence, validation, and completion; Cline owns reasoning, provider/model, continuation, and compatible interaction mechanics |
| `C:\LBE-TUI-Lab\src\main.rs`, `C:\LBE-TUI-Lab\run-lbe.bat` | Rust/Ratatui prototype/reference client; not the accepted Cline CLI entrypoint | Rust client workspace | TUI-side `Docs/00_integration_alignment.md` | Reference/integration evidence only; must not be reported as Cline CLI user-flow acceptance |
