...
class SessionOperationalHistory:
    ...
    def project_tool_receipt(self, *, session_id: str, turn_id: str, item_id: str | None, receipt: ToolReceipt, provider_tool_call_id: str | None = None, lbe_call_id: str | None = None) -> OperationalEvent:
        ...
        return self.append_event(OperationalEvent(session_id=session_id,turn_id=turn_id,item_id=item_id,event_type=event_type,payload={"tool_id":receipt.tool_id,"status":receipt.status.value,"output":dict(receipt.output or {}),"evidence":[dict(value) for value in receipt.evidence],"error_code":receipt.error_code,"error_message":receipt.error_message},provider_tool_call_id=provider_tool_call_id,lbe_call_id=lbe_call_id,runtime_operation_id=receipt.operation_id))
