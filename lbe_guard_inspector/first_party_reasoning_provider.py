...
class AnthropicReasoningBackend(ToolAwareOpenAICompatibleReasoningBackend):
    ...
    def _complete(self, stage: str, system_prompt: str, output_contract: Mapping[str, Any], output_schema: Mapping[str, Any], input_payload: Mapping[str, Any]) -> Mapping[str, Any]:
        del output_schema
        response = self._transport.post_json(
            endpoint=self._config.endpoint.strip(),
            payload={"model": self._config.model.strip(), "max_tokens": 2048, "temperature": 0, "system": system_prompt, "messages": [{"role": "user", "content": _contract_input(stage, output_contract, input_payload)}]},
            headers={"Content-Type": "application/json", "x-api-key": self._config.api_key or "", "anthropic-version": "2023-06-01"},
...
class GeminiReasoningBackend(ToolAwareOpenAICompatibleReasoningBackend):
    ...
    def _complete(self, stage: str, system_prompt: str, output_contract: Mapping[str, Any], output_schema: Mapping[str, Any], input_payload: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        response = self._transport.post_json(
            ...
            payload={"systemInstruction": {"parts": [{"text": system_prompt}]}, "contents": [{"role": "user", "parts": [{"text": _contract_input(stage, output_contract, input_payload)}]}], "generationConfig": {"temperature": 0, "responseMimeType": "application/json"}},
...
def _contract_input(stage: str, output_contract: Mapping[str, Any], input_payload: Mapping[str, Any]) -> str:
    return json.dumps({"stage": stage, "output_contract": output_contract, "input": input_payload}, ensure_ascii=False, sort_keys=True)
